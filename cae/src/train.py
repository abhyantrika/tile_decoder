...
def train(cfg: Namespace) -> None:
    ...
    dataset = custom_single(patch_size=cfg.patch_size)
    dataloader = DataLoader(
        dataset=dataset,
        batch_size=cfg.batch_size,
        shuffle=cfg.shuffle,
    ...
    for epoch_idx in range(cfg.start_epoch, cfg.num_epochs + 1):
        ...
        for batch_idx, data in enumerate(dataloader, start=1):
            ...
            avg_loss += loss.item()#avg_loss_per_image
            ...
            #     out = T.zeros(32, 33, 3, 128, 128)
            #     all_patches = dataset.patches
            #     all_patches = all_patches.reshape(32,33,3,128,128)
            #     for i in range(32):
            #         for j in range(33):
